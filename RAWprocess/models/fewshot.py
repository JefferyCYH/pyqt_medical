...
from .vgg import Encoder_SegUnet
import numpy as np
...
class FewShotSeg(nn.Module):
    ...
    """
    def __init__(self, in_channels=1, pretrained_path=None, cfg=None, train=False, ACDC=False):
        super().__init__()
        ...
        self.encoder = nn.Sequential(OrderedDict([
            ('backbone', Encoder_SegUnet(in_channels, self.pretrained_path)),]))
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs):
        ...
        for epi in range(batch_size):
            ...
            pred_full = F.interpolate(pred, size=img_size, mode='trilinear')
            outputs.append(pred_full)
        ...
        return output, align_loss / batch_size, supp_fts[1], qry_fts
    ...
    def alignLoss(self, qry_fts, pred, supp_fts, fore_mask, back_mask):
        ...
        # if self.ACDC:
        #     loss = torch.zeros(2, 80, 160, 152).cuda()
        # else:
        #     loss = torch.zeros(1, 160, 160, 160).cuda()
        loss = 0
        for way in range(n_ways):
            ...
            for shot in range(n_shots):
                ...
                supp_pred = F.interpolate(supp_pred, size=fore_mask.shape[-3:],
                                          mode='trilinear')
        ...
        # loss = torch.where(loss < 0.01, torch.full_like(loss, 0), loss)
        # loss = loss.mean()
        return loss