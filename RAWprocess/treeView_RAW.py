...
class FileSystemTreeView_RAW(QTreeView, QDockWidget):
    ...
    def select_image(self, file_index):
        ...
        if file_name.endswith('.raw'):
            src_img = np.fromfile(file_name, dtype=np.int16)
            src_img = np.reshape(src_img, (200, 160, 160))
            self.mainwindow.change_image(src_img)
        else:
            message_box = QMessageBox(QMessageBox.Warning, '提示', '数据格式错误')
            message_box.exec_()
