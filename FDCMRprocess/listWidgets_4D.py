...
from FDCMRprocess.config_4D import items


class MyListWidget_4D(QListWidget):
...
class UsedListWidget_4D(MyListWidget_4D):
    def __init__(self, parent=None):
        super().__init__(parent=parent)
    ...
    def show_attr(self):
        ...
        if type(item) in items:
            ...
            self.mainwindow.stackedWidget_4D.setCurrentIndex(index)
            self.mainwindow.stackedWidget_4D.currentWidget().update_params(param)  # 更新对应的table
            self.mainwindow.dock_attr.show()


class FuncListWidget_4D(MyListWidget_4D):
    def __init__(self, parent=None):
        super().__init__(parent=parent)
    ...
    def add_used_function(self):
        ...
        if type(func_item) in items:
            ...
            self.mainwindow.UsedListWidget_4D.addItem(use_item)
            self.mainwindow.update_image()
